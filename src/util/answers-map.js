...
function yesNoForLabel(label, a) {
  ...
  if (/(\bpreviously\b.{0,30}(work|employ))|worked (here|with us)\b|work(ed)? (here|at (this|our|the)|with us)|worked as (a |an )?(contractor|employee|intern|temp|consultant)|\b(employee|contractor|intern) (of|at)\b|ever (worked|been employed) (for|at|by)\b|former (employee|colleague|staff)|ever been (employed|an employee)|prior employment|are you a (former|returning|boomerang)|\brehire\b/.test(l)) return 'No';
...
function optionForLabel(label, options, a) {
  ...
  // alongside other job-boards/social → pick LinkedIn.
  if (/linkedin/.test(optBlob) && /(indeed|glassdoor|builtin|ziprecruiter|twitter|instagram|tiktok|\bmeta\b|facebook|job board|friend|referral|google|other)/.test(optBlob)
      && !/authoriz|sponsor|disability|veteran|gender|hispanic|race/.test(optBlob)) {
    return find(/^\s*linkedin\b/i) || find(/linkedin/i);
  }
  ...
  // Start-date / availability dropdowns: pick the soonest reasonable option
  // (2-week notice) — "2 weeks" > "immediately" > "1 month" > first non-placeholder.
  if (/start date|when.*(start|available)|availability|notice period|how soon|earliest/.test(L)) {
    return find(/2\s*weeks|two weeks|within 2|1-2 weeks/i) || find(/immediat|right away|asap|now/i) ||
           find(/1 month|30 days|2-4 weeks|3 weeks/i) || find(/1-3 months|flexible/i) ||
           options.find((o) => !/select|choose|please|^\s*$/i.test(o));
  }
  // Employment-type preference dropdowns → full-time (open to contract too).
